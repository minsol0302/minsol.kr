    ...
    try {
        ...
        try {
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    if (attempt > 0) {
                        console.log(`[Naver Auth-URL] 재시도 ${attempt}/${maxRetries}...`);
                    ...
                    if (attempt < maxRetries) {
                        console.warn(`[Naver Auth-URL] 백엔드 오류 (${response.status}), 재시도 예정...`);
                        continue;
                    }
                } catch (fetchError) {
                    ...
                    if (attempt < maxRetries) {
                        console.warn(`[Naver Auth-URL] 네트워크 오류, 재시도 예정...`, lastError.message);
                        continue;
                    }
