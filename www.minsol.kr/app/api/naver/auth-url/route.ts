...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('[Naver Auth-URL] 백엔드로 요청 전송 시작');
        ...
        console.log('[Naver Auth-URL] 백엔드 응답 상태:', response.status);

        if (!response.ok) {
            const errorText = await response.text().catch(() => '');
            console.error('[Naver Auth-URL] 백엔드 오류:', response.status, errorText);
        ...
        console.log('[Naver Auth-URL] 백엔드 응답 성공');
        return NextResponse.json(data);
    } catch (error) {
        console.error('[Naver Auth-URL] 오류 발생:', error);
        const errorMessage = error instanceof Error ? error.message : '알 수 없는 오류';
        const errorStack = error instanceof Error ? error.stack : undefined;
        console.error('[Naver Auth-URL] 오류 상세:', { errorMessage, errorStack });
